...
uint32_t DRAM_MTPS, DRAM_DBUS_RETURN_TIME,
         tRP, tRCD, tCAS;
uint32_t NVRAM_MTPS, NVRAM_DBUS_RETURN_TIME,
         NV_tRP, NV_tRCD, NV_tCAS;
...
{
    cout << "NVram_channel_width " << NVRAM_CHANNEL_WIDTH << endl
        << "NVram_wq_size " << NVRAM_WQ_SIZE << endl
        << "NVram_rq_size " << NVRAM_RQ_SIZE << endl
        << "NV_tRP " << tRP_NVRAM_NANOSECONDS << endl
        << "NV_tRCD " << tRCD_NVRAM_NANOSECONDS << endl
        << "NV_tCAS " << tCAS_NVRAM_NANOSECONDS << endl
        << "NVram_dbus_turn_around_time " << NVRAM_DBUS_TURN_AROUND_TIME << endl
        << "NVram_write_high_wm " << NVRAM_WRITE_HIGH_WM << endl
        << "NVram_write_low_wm " << NVRAM_WRITE_LOW_WM << endl
        << "min_NVram_writes_per_switch " << MIN_DRAM_WRITES_PER_SWITCH << endl
        << "NVram_mtps " << NVRAM_MTPS << endl
        << "NVram_dbus_return_time " << NVRAM_DBUS_RETURN_TIME << endl
        << endl;
}
...
void MEMORY_CONTROLLER_NV:: reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel)
{
    for (uint32_t i=0; i<queue->SIZE; i++) {
        if (queue->entry[i].scheduled) {
            ...
            uint64_t op_addr = queue->entry[i].address;
            uint32_t op_cpu = queue->entry[i].cpu,
                     op_channel = NVram_get_channel(op_addr), 
                     op_rank = NVram_get_rank(op_addr), 
                     op_bank = NVram_get_bank(op_addr), 
                     op_row = NVram_get_row(op_addr);
#ifdef DEBUG_PRINT
            //uint32_t op_column = dram_get_column(op_addr);
#endif

            // update open row
            if ((bank_request[op_channel][op_rank][op_bank].cycle_available - NV_tCAS) <= current_core_cycle[op_cpu])
                bank_request[op_channel][op_rank][op_bank].open_row = op_row;
            else
                bank_request[op_channel][op_rank][op_bank].open_row = UINT32_MAX;
...
void MEMORY_CONTROLLER_NV::operate()
{
    for (uint32_t i=0; i<NVRAM_CHANNELS; i++) {
        //if ((write_mode[i] == 0) && (WQ[i].occupancy >= DRAM_WRITE_HIGH_WM)) {
      if ((write_mode[i] == 0) && ((WQ[i].occupancy >= NVRAM_WRITE_HIGH_WM) || ((RQ[i].occupancy == 0) && (WQ[i].occupancy > 0)))) { // use idle cycles to perform writes
            write_mode[i] = 1;
...
void MEMORY_CONTROLLER_NV::schedule(PACKET_QUEUE *queue)
{
    uint64_t read_addr;
    ...
    for (uint32_t i=0; i<queue->SIZE; i++) {
        ...
        read_channel = NVram_get_channel(read_addr);
        read_rank = NVram_get_rank(read_addr);
        read_bank = NVram_get_bank(read_addr);
        ...
        read_row = NVram_get_row(read_addr);
    ...
    if (oldest_index != -1) { // scheduler might not find anything if all requests are already scheduled or all banks are busy
        ...
        if (row_buffer_hit)  
            ...
            LATENCY = NV_tRP + NV_tRCD + NV_tCAS;
...
void MEMORY_CONTROLLER_NV::process(PACKET_QUEUE *queue)
{
    ...
    uint32_t op_cpu = queue->entry[request_index].cpu,
             ...
             op_bank = NVram_get_bank(op_addr);
#ifdef DEBUG_PRINT
    uint32_t op_row = NVram_get_row(op_addr), 
             op_column = NVram_get_column(op_addr);
#endif
...
int MEMORY_CONTROLLER_NV::add_rq(PACKET *packet)
{
    ...
    uint32_t channel = NVram_get_channel(packet->address);
    int wq_index = check_NVram_queue(&WQ[channel], packet);
    if (wq_index != -1) {
    ...
    int index = check_NVram_queue(&RQ[channel], packet);
    if (index != -1)
...
#ifdef DEBUG_PRINT
            uint32_t channel = NVram_get_channel(packet->address),
                     rank = NVram_get_rank(packet->address),
                     bank = NVram_get_bank(packet->address),
                     row = NVram_get_row(packet->address),
                     column = NVram_get_column(packet->address); 
#endif
...
int MEMORY_CONTROLLER_NV::add_wq(PACKET *packet)
{
    ...
    uint32_t channel = NVram_get_channel(packet->address);
    int index = check_NVram_queue(&WQ[channel], packet);
    if (index != -1)
        return index; // merged index
...
int MEMORY_CONTROLLER_NV::add_pq(PACKET *packet)
{
    return -1;
}
...
void MEMORY_CONTROLLER_NV::return_data(PACKET *packet)
{
...
void MEMORY_CONTROLLER_NV::update_schedule_cycle(PACKET_QUEUE *queue)
{
...
void MEMORY_CONTROLLER_NV::update_process_cycle(PACKET_QUEUE *queue)
{
...
int MEMORY_CONTROLLER_NV::check_NVram_queue(PACKET_QUEUE *queue, PACKET *packet)
{
    // search write queue
...
uint32_t MEMORY_CONTROLLER_NV::NVram_get_channel(uint64_t address)
{
    if (LOG2_NVRAM_CHANNELS == 0)
        return 0;

    int shift = 0;

    return (uint32_t) (address >> shift) & (NVRAM_CHANNELS - 1);
}
...
uint32_t MEMORY_CONTROLLER_NV::NVram_get_bank(uint64_t address)
{
    if (LOG2_NVRAM_BANKS == 0)
        return 0;

    int shift = LOG2_NVRAM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVRAM_BANKS - 1);
}
...
uint32_t MEMORY_CONTROLLER_NV::NVram_get_column(uint64_t address)
{
    if (LOG2_NVRAM_COLUMNS == 0)
        return 0;

    int shift = LOG2_NVRAM_BANKS + LOG2_NVRAM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVRAM_COLUMNS - 1);
}
...
uint32_t MEMORY_CONTROLLER_NV::NVram_get_rank(uint64_t address)
{
    if (LOG2_NVRAM_RANKS == 0)
        return 0;

    int shift = LOG2_NVRAM_COLUMNS + LOG2_NVRAM_BANKS + LOG2_NVRAM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVRAM_RANKS - 1);
}
...
uint32_t MEMORY_CONTROLLER_NV::NVram_get_row(uint64_t address)
{
    if (LOG2_NVRAM_ROWS == 0)
        return 0;

    int shift = LOG2_NVRAM_RANKS + LOG2_NVRAM_COLUMNS + LOG2_NVRAM_BANKS + LOG2_NVRAM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVRAM_ROWS - 1);
}
...
uint32_t MEMORY_CONTROLLER_NV::get_occupancy(uint8_t queue_type, uint64_t address)
{
    uint32_t channel = NVram_get_channel(address);
    if (queue_type == 1)
...
uint32_t MEMORY_CONTROLLER_NV::get_size(uint8_t queue_type, uint64_t address)
{
    uint32_t channel = NVram_get_channel(address);
    if (queue_type == 1)
...
void MEMORY_CONTROLLER_NV::increment_WQ_FULL(uint64_t address)
{
    uint32_t channel = NVram_get_channel(address);
    WQ[channel].FULL++;
}
